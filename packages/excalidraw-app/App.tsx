...
import type { ResolvablePromise } from "../excalidraw/utils";
import {
  debounce,
  getVersion,
  getFrame,
...
import type { RestoredDataState } from "../excalidraw/data/restore";
import { restore, restoreAppState } from "../excalidraw/data/restore";
import {
...
import "./index.scss";
import type { ResolutionType } from "../excalidraw/utility-types";
...
import type { RemoteExcalidrawElement } from "../excalidraw/data/reconcile";
import {
  CommandPalette,
  DEFAULT_CATEGORIES,
} from "../excalidraw/components/CommandPalette/CommandPalette";
import {
  GithubIcon,
  XBrandIcon,
  DiscordIcon,
  ...
  share,
  youtubeIcon,
} from "../excalidraw/components/icons";
...
const ExcalidrawWrapper: React.FC<ExcalidrawAppProps> = (props) => {
  ...
  const roomKey = collabDetails?.roomKey;
  const children = excalidraw?.children;
  ...
  const startCollaboration = (
    ...
    collabAPI.startCollaboration(
      roomId && roomKey ? { roomId, roomKey } : null,
    );
...
const ExcalidrawApp: React.FC<ExcalidrawAppProps> = (props) => {
  const { excalidraw } = props;
  const children = excalidraw?.children;
  return (
    <TopErrorBoundary>
      <Provider unstable_createStore={() => appJotaiStore}>
        <ExcalidrawWrapper {...props}>{children}</ExcalidrawWrapper>
      </Provider>
    </TopErrorBoundary>
  );
};
